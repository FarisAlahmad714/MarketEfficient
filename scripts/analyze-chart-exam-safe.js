...
// scripts/analyze-chart-exam-safe.js
// SAFE READ-ONLY ANALYSIS - No data changes made
  ...
  try {
    ...
    for (const result of chartExamResults) {
      ...
      const sample = {
        ...
        completedAt: result.completedAt?.toISOString?.() || 'Unknown'
      };
...
if (require.main === module) {
  analyzeChartExamData()
    .then((result) => {
      colorLog('green', '✅ Analysis complete');
      process.exit(0);
    })
    .catch((error) => {
