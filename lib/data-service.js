...
export async function fetchCryptoOHLCData(apiId, timeframe = 'daily', days = 30, startDate = null, seed = Date.now()) {
  ...
  try {
    ...
    if (dataCache.failedRequests.crypto < 3 || startDate) {
      ...
      console.log('Making TwelveData API request...');
      const response = await axios.get(url, { headers: { 'Accept': 'application/json' }, timeout: 15000 });
      console.log('TwelveData API response status:', response.status);
      console.log('TwelveData API response data:', JSON.stringify(response.data).substring(0, 500));
      ...
      ohlcData = response.data.values.map(item => {
        ...
        try {
          // TwelveData returns dates like "2025-07-04 00:00:00"
          // Need to handle this format properly
          if (dateStr.includes(' ')) {
          ...
          if (isNaN(date.getTime())) {
            throw new Error(`Invalid date: ${item.datetime}`);
          }
          
          finalDate = date.toISOString();
        } catch (e) {
          console.error('Date parsing error:', e.message, 'Original date:', item.datetime);
...
export async function fetchAssetOHLCData(asset, timeframe = 'daily', candles = 25, startDate = null, seed = Date.now()) {
  try {
    let ohlcData = [];
    console.log('fetchAssetOHLCData called with:', {
...
export function generateMockOHLCData(basePrice, count = 20, timeframe = 'daily', startDate = null, seed = Date.now()) {
  ...
  for (let i = 0; i < count; i++) {
    // Calculate date for this candle
    const baseTime = (startDate || now).getTime();
    const candleOffset = (count - 1 - i) * timeIncrement;
    
    // Ensure we don't create an invalid date
    let dateTime = baseTime - candleOffset;
    
    // Make sure the date is valid (not too far in the past or future)
    const minValidDate = new Date('1970-01-01').getTime();
    const maxValidDate = new Date('2100-01-01').getTime();
    
    if (dateTime < minValidDate || dateTime > maxValidDate || isNaN(dateTime)) {
      // Use a safe fallback date
      dateTime = now.getTime() - candleOffset;
    }
    ...
    // Validate date after modifications
    if (isNaN(date.getTime())) {
      // Reset to a valid date if something went wrong
      date.setTime(now.getTime() - (count - 1 - i) * timeIncrement);
    }
    ...
    try {
      ...
      const fallbackDate = new Date(now.getTime() - (count - 1 - i) * timeIncrement);
