    ...
    if (dataCache.failedRequests.crypto < 3) {
      ...
      if (interval === 'hourly' || interval === 'daily') {
        for (let i = 0; i < prices.length; i++) {
          ...
          // Get volume if available, otherwise use a calculated value
          const volume = total_volumes && i < total_volumes.length ? total_volumes[i][1] : closePrice * 1000 * (0.5 + Math.random());
      ...
      if (timeframe === 'weekly' || timeframe === 'monthly') {
        ...
        for (let i = 0; i < prices.length; i++) {
          ...
          // Get volume for the current timestamp
          const volume = total_volumes && i < total_volumes.length ? total_volumes[i][1] : price * 1000 * (0.5 + Math.random());
          ...
          if (!periodData[periodKey]) {
            ...
            periodData[periodKey].volume += volume; // Accumulate volume within the period
          }
        }

        ohlcData = Object.values(periodData).map(({ date, open, high, low, close, volume }) => ({
          date: new Date(date).toISOString(), open, high, low, close, volume
...
export async function fetchStockOHLCData(symbol, timeframe = 'daily', candles = 30, startDate = null, seed = Date.now()) {
  ...
  if (Math.random() > 0.1 || dataCache.failedRequests.stock >= 2) {
    logger.log(`Using mock data for ${symbol} to avoid API rate limits`);
