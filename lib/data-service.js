...
export async function fetchCryptoOHLCData(apiId, timeframe = 'daily', days = 30, startDate = null, seed = Date.now()) {
  ...
  if (timeSinceLastRequest < 500) {
    ...
    logger.log(`Rate limiting: waiting ${waitTime}ms before next TwelveData API call`);
  ...
  try {
    if (dataCache.failedRequests.crypto < 3) {
      ...
      // Convert CoinGecko IDs to TwelveData crypto symbols
      ...
      const interval = timeframe === '4h' ? '4h' : timeframe === 'daily' ? '1day' : timeframe === 'weekly' ? '1week' : timeframe === 'monthly' ? '1month' : '1day';
