...
export async function fetchCoinGeckoOHLC(coinId, days, vs_currency = 'usd') {
  ...
  try {
    ...
    // Generate mock data for Plotly if we can't fetch real data
...
function generateMockPlotlyData(coinId, days) {
  ...
  // Use seed based on coinId for consistency
  const seed = coinId.split('').reduce((a, b) => a + b.charCodeAt(0), 0);
  const random = seedRandom(seed);
  
  // Generate some reasonable base price based on the coin
  let basePrice;
  if (coinId === 'bitcoin') {
  ...
  const trend = random() > 0.5 ? 0.001 : -0.001; // Slight up or down trend
  
  // Get the current date and go back 'days' days
  ...
  for (let date = startDate; date <= endDate; date.setDate(date.getDate() + 1)) {
    // Generate the day's price action
