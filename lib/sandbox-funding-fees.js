...
class SandboxFundingFeeManager {
  ...
  async processFundingFees() {
    try {
      ...
      const now = new Date();
      console.log(`Processing funding fees at ${now.toISOString()}`);
      ...
      if (leveragedTrades.length === 0) {
        console.log('No leveraged positions requiring funding fees');
      ...
      for (const trade of leveragedTrades) {
        try {
          ...
          console.error(`Error processing funding fee for trade ${trade._id}:`, error);
        }
      }

      console.log(`Processed funding fees: ${tradesProcessed} trades, total fees: ${totalFeesProcessed.toFixed(6)} SENSES`);
      ...
      console.error('Error processing funding fees:', error);
      throw error;
    }
  }
  ...
  async calculateAndApplyFundingFee(trade) {
    try {
      ...
      console.log(`Applied funding fee: ${trade.symbol} ${trade.side} ${trade.leverage}x - ${totalFundingFee.toFixed(6)} SENSES (${periodsToCharge} periods)`);
      
      return totalFundingFee;

    } catch (error) {
      console.error(`Error calculating funding fee for trade ${trade._id}:`, error);
      throw error;
    }
  }
