...
export const uploadImageToGCS = async (base64String, destinationFolder = 'profile-images') => {
  ...
  // Log the beginning of the base64 string for debugging
  if (base64String) {
    logger.log('Base64 string starts with:', base64String.substring(0, 50));
    logger.log('Base64 string length:', base64String.length);
  }
  
  if (!base64String || (!base64String.startsWith('data:image') && !base64String.startsWith('data:application/octet-stream'))) {
    throw new Error(`Invalid base64 image string provided. String starts with: ${base64String ? base64String.substring(0, 50) : 'null'}`);
  }
  ...
  try {
    ...
    // For HEIC files, the browser might set the MIME type as application/octet-stream
    // So we need to check the actual file content
    const isOctetStream = mimeType === 'application/octet-stream';
    
    if (!mimeType || (!mimeType.startsWith('image/') && !isOctetStream)) {
    ...
    if (!allowedTypes.includes(mimeType) && !isOctetStream) {
    ...
    // Determine the actual file type and extension
    let actualMimeType = mimeType;
    let extension = mimeType.split('/')[1];
    
    // If MIME type is octet-stream, detect the actual type from file content
    if (isOctetStream) {
      ...
      if (buffer.length > 12) {
        ...
        if (ftypHeader === 'ftyp' && heifBrands.includes(brandCode)) {
          actualMimeType = 'image/heic';
          extension = 'heic';
        } else {
          throw new Error('Unable to determine image type from file content');
        }
      }
    }
    
    // Security: Basic image header validation
    if (!isValidImageBuffer(buffer, actualMimeType)) {
