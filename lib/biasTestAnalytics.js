...
export async function processBiasTestAnalytics(testResult, deviceInfo = {}) {
  try {
    ...
    for (let i = 0; i < testDetails.length; i++) {
      ...
      const technicalFactors = extractTechnicalFactors(question.reasoning || '');
      
      // Calculate technical complexity score
      const technicalComplexity = calculateTechnicalComplexity(technicalFactors, question.reasoning);
      
      // Calculate confidence accuracy
      const confidenceAccuracy = calculateConfidenceAccuracy(question.confidenceLevel, question.isCorrect);
      
      questionAnalytics.push({
        questionNumber: i + 1,
        timeSpent: question.timeSpent || 60,
        prediction: question.prediction?.toLowerCase() || 'bullish',
        correctAnswer: question.correctAnswer?.toLowerCase() || 'bullish',
        isCorrect: question.isCorrect || false,
        confidenceLevel: question.confidenceLevel || 5,
        confidenceAccuracy: confidenceAccuracy,
        reasoning: question.reasoning || '',
        reasoningLength: (question.reasoning || '').length,
        technicalFactors: technicalFactors,
        technicalComplexity: technicalComplexity,
        marketCondition: question.marketCondition || 'trending',
        volumeProfile: question.volumeProfile || {
          avgVolume: 0,
          volumeTrend: 'stable',
          volumeSpikes: 0
        },
