 ...
 */
export async function logTradingError(error, userId, action, details = {}) {
  try {
    ...
    const auditData = {
      adminId: 'SYSTEM', // System-generated error
      action: `TRADING_ERROR_${action.toUpperCase()}`,
      category: 'Trading',
 ...
 */
export async function handleMarketDataError(symbol, error, userId, fallbackPrice = null) {
 ...
 */
export async function handlePositionError(operation, tradeId, error, userId, context = {}) {
 ...
 */
export async function handleDatabaseError(operation, error, userId, context = {}) {
 ...
 */
export async function handleLiquidationError(tradeId, error, userId, context = {}) {
 ...
 */
export function tradingErrorMiddleware(error, req, res, next) {
  // Log the error
  if (req.user?.id) {
    logTradingError(
...
export const marketDataCircuitBreaker = new CircuitBreaker(5, 60000); // 5 failures, 1 minute timeout
export const databaseCircuitBreaker = new CircuitBreaker(3, 30000); // 3 failures, 30 second timeout