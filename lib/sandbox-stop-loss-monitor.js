...
class SandboxStopLossMonitor {
  ...
  async monitorLoop() {
    ...
    while (this.isRunning) {
      try {
        console.log('[Stop Loss Monitor] Running scheduled check...');
  ...
  async checkAllOpenTrades() {
    try {
      console.log('[Stop Loss Monitor] Connecting to database...');
      await connectDB();
      console.log('[Stop Loss Monitor] Database connected successfully');
      ...
      console.log(`[Stop Loss Monitor] Checking ${openTrades.length} open trades at ${new Date().toISOString()}`);
      ...
      for (const trade of openTrades) {
        ...
        console.log(`[Stop Loss Monitor] ${trade.symbol}: Current price ${currentPrice}, SL: ${trade.stopLoss?.price}, TP: ${trade.takeProfit?.price}`);
