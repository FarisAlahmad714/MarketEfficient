...
class SecureStorage {
  ...
  async initializeKey() {
    ...
    if (this._cachedSessionKey) {
      try {
        ...
        console.error("[SecureStorage] Failed to derive key from cached sessionKey:", error);
    ...
    try {
      console.log("[SecureStorage] Initializing key. Fetching session key from API...");
      ...
      console.log("[SecureStorage] Key initialized successfully.");
    } catch (error) {
      console.error("[SecureStorage] Failed to initialize key:", error);
      // Handle key initialization failure, e.g. by disabling storage or notifying user
    }
  }

  async deriveKey(sessionKey) {
    // Securely derive a key using PBKDF2
    // The salt should ideally be unique and stored/transmitted securely if needed,
    // or be a fixed, application-specific value if the sessionKey itself is highly entropic.
    // For this example, using a fixed salt. Consider generating and storing a salt per user if possible.
    const salt = CryptoJS.enc.Hex.parse("your-application-specific-salt"); // IMPORTANT: Change this to a unique, random salt for your app
    const key = CryptoJS.PBKDF2(sessionKey, salt, {
  ...
  encrypt(data) {
    if (!this.encryptionKey) {
      console.error('[SecureStorage] Encryption key not initialized.');
      return null;
    }
    try {
  ...
  decrypt(encryptedData) {
    ...
    try {
      if (typeof encryptedData !== 'string' || !encryptedData.includes('.')) {
        ...
        console.warn('[SecureStorage] Data is not in expected encrypted.hmac format.');
  ...
  async setItem(key, value) {
    if (typeof window === 'undefined' || !this.encryptionKey) {
      if (!this.encryptionKey) {
        console.error('[SecureStorage] Cannot set item, key not initialized. Attempting to initialize...');
        await this.initializeKey();
        if (!this.encryptionKey) {
          console.error('[SecureStorage] Key initialization failed. Cannot set item.');
          return;
        }
      } else {
    ...
    try {
      ...
      if (!encrypted) {
        console.error('[SecureStorage] Encryption failed - not storing data for key:', key);
  ...
  async getItem(key) {
    ...
    if (!this.encryptionKey) {
      console.error('[SecureStorage] Cannot get item, key not initialized. Attempting to initialize...');
      await this.initializeKey();
      if(!this.encryptionKey) {
          console.error('[SecureStorage] Key initialization failed. Cannot get item.');
          return null;
      }
    }
    ...
    try {
      ...
      if (!encrypted) {
        ...
        if (unsecureData) {
            console.warn(`[SecureStorage] Retrieved potentially unencrypted data for key "${key}" from localStorage.`);
  ...
  clear() {
    ...
    if (process.env.NODE_ENV === 'development') {
        console.log('[SecureStorage] Cleared all secure items.');
    }
  }
...
if (typeof window !== 'undefined') {
  secureStorageInstance.initializeKey().catch(err => {
    console.error("Error during initial key initialization:", err);
