...
const VALIDATION_LIMITS = {
  ...
  LEVERAGE: {
    MIN: 1,
    MAX: 20  // Maximum leverage across all assets, individual assets may have lower limits
  },
  PRICE: {
    MIN: 0.01,
    MAX: 10000000
 ...
 * Validate leverage
 */
function validateLeverage(leverage, symbol = null) {
  ...
  // Get asset-specific maximum leverage if symbol provided
  let maxLeverage = VALIDATION_LIMITS.LEVERAGE.MAX;
  if (symbol) {
    const upperSymbol = symbol.toUpperCase();
    const allAssets = [...SANDBOX_ASSETS.crypto, ...SANDBOX_ASSETS.stocks];
    const asset = allAssets.find(a => a.symbol === upperSymbol);
    if (asset && asset.maxLeverage) {
      maxLeverage = asset.maxLeverage;
    }
  }

  if (numLeverage < VALIDATION_LIMITS.LEVERAGE.MIN || numLeverage > maxLeverage) {
    throw new ValidationError(
      `Leverage must be between ${VALIDATION_LIMITS.LEVERAGE.MIN}x and ${maxLeverage}x${symbol ? ` for ${symbol}` : ''}`,
      'leverage',
...
function validateTradeRequest({
  ...
  try {
    ...
    // Validate leverage with asset-specific limits
    validatedData.leverage = validateLeverage(leverage, validatedData.symbol);
