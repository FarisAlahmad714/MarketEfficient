...
export const getBadgeObjectsFromIds = async (userId, badgeIds) => {
  ...
  try {
    // Get all possible badges for this user
    const allBadges = await generateUserAchievements(userId);
    
    // Filter to only include badges that the user has earned
    const earnedBadges = allBadges.filter(badge => badgeIds.includes(badge.id));
    
    return earnedBadges;
  } catch (error) {
    console.error('Error converting badge IDs to objects:', error);
    return [];
  }
};
