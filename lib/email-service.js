...
async function sendEmail(to, subject, htmlContent, textContent) {
  ...
  try {
    ...
    // Create transporter using Mailjet SMTP
    const transporter = nodemailer.createTransport({
      host: "in-v3.mailjet.com",
      port: 587,
      secure: false, // TLS
      auth: {
        user: process.env.MAILJET_API_KEY, // Your API Key: 59e33022190a1b002fe562262c9068f7
        pass: process.env.MAILJET_SECRET_KEY // Your Secret Key: ca4a0656b1bcb748353d14ae609ba0dd
      }
    });

    const fromName = process.env.EMAIL_SENDER_NAME || 'ChartSense';
    const fromEmail = process.env.EMAIL_SENDER_EMAIL; // Using farisalahmad714@gmail.com
    ...
    console.log('Email sent successfully via Mailjet:', info.messageId);
    return true;
  } catch (error) {
