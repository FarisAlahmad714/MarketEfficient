...
export class AdminSecurityValidator {
  ...
  async validateAdminTrade(userId, tradeData, justification = null) {
    ...
    try {
      ...
      console.error('Admin security validation error:', error);
      return {
        allowed: false,
  ...
  async checkDailyVolume(userId, tradeData) {
    try {
      ...
      console.error('Daily volume check error:', error);
      return {
        allowed: true,
...
// Global instances
export const adminSecurityValidator = new AdminSecurityValidator();
