 ...
 * @param {number} params.quantity - Position quantity (already includes leverage effect)
 * @param {number} params.totalFees - Total fees paid (optional, defaults to 0)
 * @returns {number} Unrealized P&L in SENSES
 */
function calculateUnrealizedPnL({ side, entryPrice, currentPrice, quantity, totalFees = 0 }) {
  // Validate inputs
  ...
  // P&L = price difference * quantity - fees
  // Note: quantity already includes leverage effect from position sizing
 ...
 */
function calculateRealizedPnL({ side, entryPrice, exitPrice, quantity, totalFees = 0 }) {
